...
{
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"signup_bg.jpg"]];
    _backButtonRef.alpha = 0.0;
    _ccpatch.alpha = 0.0;
...
-(IBAction)signupclicked:(id)sender
{
    ...
    _Number.alpha = 1.0;
    _Email1.alpha = 1.0;
    _loginLabel.text = @"Enter details below to sign up";
...
//    
//    
//	NSString* shareText = [NSString stringWithFormat:@"Just downloaded the latest Besharam Official App from http://tinyurl.com/off-yjhd"];
//	
//	
//	
//	//  Create an instance of the Tweet Sheet
	...
	reach.reachableBlock = ^(Reachability * reachability)
	{
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			if ([responseD isEqualToString:@"fail"] || [responseD isEqualToString:@"success"] || [responseD isEqualToString:@"FAIL"] ) {
				...
				[Overlaybtn setBackgroundImage:[UIImage imageNamed:@"iphone-5_overlay.jpg"] forState:UIControlStateNormal];
				//[Coverflow[i] setBackgroundImage:CoverImage[i] forState:UIControlStateHighlighted];
