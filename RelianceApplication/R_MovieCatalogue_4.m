...
@interface R_MovieCatalogue_4 ()

@end

@implementation R_MovieCatalogue_4
...
-(void)scrollTracker
{
    ...
    for (int i=0; i<TotalData; i++) {
        if (Movie_Scroller.contentOffset.y == 323*i) {
            for (int i=0; i<TotalData; i++) {
...
-(void)DisplayMovie: (UIButton *)sender
{
    ...
    Movie_Scroller			= [[UIScrollView alloc] initWithFrame:CGRectMake(0, 93, 320,416-93)];
...
if ([data14[i] isEqualToString:data11[sender.tag]])
{
    if([data11[sender.tag] isEqualToString:@"1"] | [data11[sender.tag]isEqualToString:@"2"])
    {
        ...
        PAGINGEFFECT[pagingCount]  = [[UIImageView alloc] initWithFrame:CGRectMake(296+10, n+120, 12,13)];
        ...
        if (movieImage[i] == nil)
        {
            movieImage[i] = [[AsyncImageView alloc] initWithFrame:CGRectMake(20,m, 280, 301-10)];
        ...
        mvDetailButton.frame = CGRectMake(320-117-40, 291-40, 117, 39);
        ...
        mvDetailButton.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"more_btn.png"]];        [movieImage[i] addSubview:mvDetailButton];
        
        moviName[i] = [[UITextView alloc] initWithFrame:CGRectMake(2, 291+m, 311, 32)];
    ...
    if([data11[sender.tag] isEqualToString:@"1"] | [data11[sender.tag]isEqualToString:@"3"])
    {
    ...
    if([obj.CHECK_CATALOG isEqualToString:@"1"])
    {
        ...
        [Overlaybtn setBackgroundImage:[UIImage imageNamed:@"catelog_overlay4.jpg"] forState:UIControlStateNormal];
...
-(void)openMovieDetails:(UITapGestureRecognizer *)sender
{
    ...
    R_OpenMovie_4 *openDetails = [[R_OpenMovie_4 alloc]initWithNibName:@"R_OpenMovie_4" bundle:nil];
