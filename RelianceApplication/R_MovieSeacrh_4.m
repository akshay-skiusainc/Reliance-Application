...
@interface R_MovieSeacrh_4 ()

@end

@implementation R_MovieSeacrh_4
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self getdata];
    GalleryScroll			= [[UIScrollView alloc] initWithFrame:CGRectMake(0, 93+37, 320,416-130)];
    ...
    //	[GalleryScroll release];
	
    
    //    NSString *insertSQL5 = [NSString stri ngWithFormat: @"SELECT * FROM PImages where PROPERTYID=%@",pid[1]];
    //	[self displayImages:insertSQL5];
...
-(void)displaymovies
{
	...
	for (int i =0; i<TotalData; i++)
    {
		...
		nX=110*(i%3);
		nY=170 * (i/3);
        //		frame[i]					= [[UIImageView alloc] initWithFrame:CGRectMake(nX+8, nY+0, 2   26,180)];
        //		frame[i].image = [UIImage imageNamed:@"frame.png"];
