...
@interface iPhone5FacebookViewController ()

@end

@implementation iPhone5FacebookViewController
@synthesize footerView,singleFeedScroll;
@synthesize FbFeedLable,bigImageView,movieNameLabel,dateBigLabel,descLabel,socialFeedsBgView,singleScrollImage;
...
- (void)viewDidLoad
{
	[super viewDidLoad];
    [self getsharetext];
    self.trackedViewName=@"Facebook Feed";
...
-(void)DisplayFeeds
{
	...
	VideoScroller			= [[UIScrollView alloc] initWithFrame:CGRectMake(0,40,320,440)];
	...
	for (int i =0; i<TotalNumOfRows; i++)
    {
		...
		ImagePatch[i] = [[UIImageView alloc] initWithFrame:CGRectMake(6, m+4, 311,312)];
		...
		socializeBg[i] = [[UIView alloc]initWithFrame:CGRectMake(15, 262+m,291,44)];
...
//		
//		UIImageView *img= [[UIImageView alloc] initWithFrame:CGRectMake(150, 260, 150, 44)];
...
//			//	u+=35;
//		}
//		Publish[0].frame = CGRectMake(70+100, 262, 32, 32);
//		Publish[1].frame = CGRectMake(110+100, 262, 32, 32);
//		Publish[2].frame = CGRectMake(150+100, 262, 32, 32);
//
//		Publish[0].tag= i;
...
- (void)OpenFeed:(id)sender
{
    ...
    NSLog(@"Index for open feed  %d",index);
    ...
    if (actionBarDetail == nil)
    {
        ...
        actionBarDetail.itemsRight = [NSArray arrayWithObjects:[SZActionButton viewsButton], [actionBarDetail.itemsRight objectAtIndex:0], [actionBarDetail.itemsRight objectAtIndex:1], nil];
        // actionBar[i].itemsLeft = [NSArray arrayWithObjects:nil];
...
//               [NSTimer scheduledTimerWithTimeInterval:3.0 target:self selector:@selector(fetchall) userInfo:nil repeats:YES];
            // Handle error.
        }
        else
        {
