...
@interface iPhone5ShortScenesVC ()

@end

@implementation iPhone5ShortScenesVC
...
-(void)getAllData{
	...
	else
	{
        ...
        if (!errorReturned) {
            ...
            NSLog(@" view controller json=%@",json);
            ...
            obj.xtraDictionary = trailers;
            NSLog(@"obj.trailers in viewController == %@",obj.trailers);
	...
	for (int i =0; i<TotalData; i++)
    {
		if([data6[i] isEqualToString:movieSearch.text])
        {
                ...
                
            {
                ...
                webwall[i] = [[AsyncImageView alloc] initWithFrame:CGRectMake(8, 30, 308,180)];
